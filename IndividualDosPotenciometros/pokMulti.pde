...
void setup() {
  ...
  pokeball2 = loadImage("pokeball2.png");
...
void draw() {
  ...
  if (0 < port.available())
  {
    ...
    if(sensores != null)
    {
      ...
      if(datosSensor.length == 2) //tamaño de arreglo segun datos
      {
    ...
    mapeado = map(potenciometro1,0,1023,0,800);
    mapeado2 = map(potenciometro2,0,1023,0,800);
  }
  ...
  if (gameOver) {
    ...
    textAlign(CENTER);
    fill(200, 160, 43);
    text("Perdio", width/2, height/2);
  } else {

    // Localización catcher
    catcher.setLocation(mapeado+10, 400);
    catcher2.setLocation(mapeado2+5, 400);
    // Display the catcher
    catcher.display();
    catcher2.display();
